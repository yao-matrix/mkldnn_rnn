...
TEST(MkldnnRNNOpsTest, ParamsSize_ShapeFn) {
  ShapeInferenceTestOp op("MkldnnRNNParamsSize");
  INFER_OK(op, "[1];[1];[1]", "[1]");
}

TEST(MkldnnRNNOpsTest, ForwardLstm_ShapeFn) {
  ShapeInferenceTestOp op("MkldnnRNN");
  TF_ASSERT_OK(NodeDefBuilder("test", "MkldnnRNN")
